...
function send_int(value,str)
   str = str .. string.char(bit:_and(bit:_rshift(value,24),255)) .. string.char(bit:_and(bit:_rshift(value,16),255)) .. string.char(bit:_and(bit:_rshift(value,8),255)) .. string.char(bit:_and(value,255))
   return str
end

function send_word(value,str)
   str = str  .. string.char(bit:_and(bit:_rshift(value,8),255)) .. string.char(bit:_and(value,255))
   return str
end
...
function send_qword(value,str)
   str=send_int ( bit:_rshift(value,32) , str );
   str=send_int ( bit:_and(value, 0xffffffff) ,str );
   return str
end
...
function sphinx.sphinx_set_server(self,host,port)
	...
	self.client.sock = assert(socket.connect(host, port))
end
...
function sphinx.sphinx_query(self,query,index_list, comment)
    ...
    assert(self.client.sock:send(input))
	
	local input, recvt, sendt, status,response, receive_status
	recvt, sendt, status = socket.select({self.client.sock}, nil, 1)
	...
	assert(self.client.sock:send(req_header))
	assert(self.client.sock:send(req))
...
client:sphinx_set_server("127.0.0.1",9312)
...
print(string.format("Query '%s' retrieved %d of %d matches.\n",query, res.total, res.total_found))

	--thelen = string.len(response)
    --for i=1,thelen do
    --  print(string.byte(response,i) )
    --end

print ( "Query stats:\n" )
for  i=1,res.num_words do
	print ( string.format("\t'%s' found %d times in %d documents\n",
...
for i=1, res.num_matches do
	print ( "\n" );
	print ( string.format("%d. doc_id=%d, weight=%d", i,res.matches[i].doc,res.matches[i].weight) )
	print ( "attrs:" );
	for j=1,res.num_attrs do
		   print(res.attr_names[j],res.matches[i].attr[j].value)
	end
